...
class App extends Component {
  constructor() {
    ...
    this.state = {
      ...
      quizLists: [{
        quizname: 'JAVASCRIPT',
        quizNumber: ["Quiz 1", "Quiz 2", "Quiz 3"]
      },
      {
        quizname: 'CSS',
        quizNumber: ["Quiz 1", "Quiz 2", "Quiz 3"]
    ...
    this.moreQuiz = this.moreQuiz.bind(this)
  }
  async updateText(e) {
  ...
  async showResult(quizScore, quizLength, totalCorrect) {
  ...
  render() {
    ...
    return (
      <div>
        {userFlag && validFlag && < button className="btn btn-warning logOut" onClick={this.logOut}>Log Out</button>}
        ...
        {userFlag && validFlag && !quizInfoFlag && !quizShowFlag && <QuizInfo quizQuestion={quizQuestion} quizList={quizLists} showTest={this.showTest} testIndex={currentIndex} testInnerIndex={currentQuizIndex} testGiven={testGiven} score={score} />}
        ...
        {userFlag && validFlag && resultFlag && resultShowFlag && <Result moreQuiz={this.moreQuiz} score={score} correct={correct} questionLength={questionLength} />}
      </div >
    );
  }
}
